...
import { 
  ...
  AlignLeft, Bell, Star, Layers, Globe, PhoneCall, MapPin, Briefcase, ClipboardList
} from 'lucide-react';
...
function App() {
  ...
  const ModalWrapper = ({ title, children, onClose }) => (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex justify-center items-end sm:items-center">
      <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl h-[85vh] sm:h-auto max-h-[90vh] flex flex-col animate-slide-up shadow-2xl">
    ...
    return (
      <div className="h-full flex flex-col">
         ...
         <div className="flex-1 overflow-hidden">
            ...
            {adminView === 'expenses' && <ExpensesView />}
            {adminView === 'bulk' && <BulkOpsView />}
  ...
  const UserManagementView = () => {
     return (
        <div className="h-full flex flex-col p-4 pb-24">
           <div className="flex justify-between items-center mb-4">
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest">Team Performance</h3>
           ...
           <div className="space-y-3 overflow-y-auto flex-1 pr-1">
              {appUsers.map(u => {
                 const userInvoices = invoices.filter(inv => inv.salespersonId === u.id && inv.status === 'Billed');
                 ...
                 return (
                    <div key={u.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                       <div className="flex justify-between items-start mb-2">
                          <div><h4 className="font-bold text-slate-800 text-base">{u.name}</h4><span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded mt-1 inline-block border ${u.role === 'admin' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>{u.role}</span></div>
                          <div className="flex gap-1.5"><button onClick={() => { setEditingUser(u); setShowUserModal(true); }} className="p-2 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-100 transition-colors"><Edit size={16}/></button><button onClick={async () => { if(window.confirm(`Permanently delete user ${u.name}?`)) await deleteFromFirebase('app_users', u.id); }} className="p-2 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 transition-colors"><Trash2 size={16}/></button></div>
                       </div>
                       <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
                          <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Billed Invoices</p><p className="font-extrabold text-slate-700 text-lg">{userInvoices.length}</p></div>
                          <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total Revenue generated</p><p className="font-extrabold text-emerald-600 text-lg">Rs. {totalSales.toLocaleString()}</p></div>
                       </div>
                    </div>
                 );
              })}
  ...
  const AnalyticsView = () => {
    ...
    const reportEngine = useMemo(() => {
      let billedForPnL = invoices.filter(o => o.status === 'Billed' && checkDateFilter(o.date, dateFilter));
      ...
      customers.forEach(c => { const bal = getCustomerBalance(c.id); if(bal > 0) { kpis.totalReceivables += bal; receivablesList.push({ name: c.name, amount: bal }); } });
      billedForPnL.forEach(o => {
        ...
        o.items.forEach(item => {
          ...
          if(filterCompany && itemCompanyId !== Number(filterCompany)) return;
          ...
          if(!byCompany[item.company]) byCompany[item.company] = { qty: 0, revenue: 0, cost: 0, profit: 0 };
          byCompany[item.company].qty += item.quantity; byCompany[item.company].revenue += itemRev; byCompany[item.company].cost += itemCost; byCompany[item.company].profit += (itemRev - itemCost);
        });
      ...
      const filteredExpenses = expenses.filter(e => checkDateFilter(e.date, dateFilter)).reduce((sum, e) => sum + Number(e.amount), 0);
      kpis.totalExpenses = filteredExpenses;
      ...
      invoices.filter(o => o.status === 'Billed' && o.date >= prevPeriod.start && o.date <= prevPeriod.end).forEach(o => {
        o.items.forEach(item => { prevRevenue += item.price * item.quantity; prevProfit += (item.price - item.costPrice) * item.quantity; });
      });
      const trends = { revenue: prevRevenue > 0 ? ((kpis.productRevenue - prevRevenue) / prevRevenue * 100).toFixed(1) : 0, profit: prevProfit > 0 ? ((kpis.grossMargin - prevProfit) / prevProfit * 100).toFixed(1) : 0 };
      const dailyBreakdown = {};
      billedForPnL.forEach(o => {
        if (!dailyBreakdown[o.date]) dailyBreakdown[o.date] = { revenue: 0, profit: 0, orders: 0 };
        let dayRevenue = 0, dayCost = 0;
        o.items.forEach(item => { dayRevenue += item.price * item.quantity; dayCost += item.costPrice * item.quantity; });
        dailyBreakdown[o.date].revenue += dayRevenue; dailyBreakdown[o.date].profit += (dayRevenue - dayCost); dailyBreakdown[o.date].orders += 1;
      });
      return { kpis, byProduct, byCompany, byCustomer, receivablesList: receivablesList.sort((a,b)=>b.amount - a.amount), trends, dailyBreakdown };
    }, [invoices, expenses, dateFilter, filterCompany, filterCustomer, products, customers]);
    const getSortedExportData = () => {
       if (view === 'Overview') return null;
       if (view === 'Receivables') return reportEngine.receivablesList.map(r => ({ Name: r.name, Amount: r.amount }));
...
\
`;
            if (view === 'Overview') { text += `Product Sales: Rs. ${reportEngine.kpis.productRevenue}\
Total COGS: Rs. ${reportEngine.kpis.totalCOGS}\
Gross Margin: Rs. ${reportEngine.kpis.grossMargin}\
Delivery Billed: Rs. ${reportEngine.kpis.deliveryBilled}\
Transport Exp: Rs. ${reportEngine.kpis.transportExpense}\
Operational Exp: Rs. ${reportEngine.kpis.totalExpenses}\
*Net Profit: Rs. ${reportEngine.kpis.netProfit}*\
`; }
            else { exportData.forEach((r, i) => { text += `${i+1}. *${r.Name}* ${r.Company ? `(${r.Company})` : ''}\
   Qty: ${r.Qty} | Rev: Rs.${r.Revenue} | Profit: Rs.${r.GrossProfit || r.Amount}\
`; }); }
    ...
    const renderTable = (dataObj, type) => {
      ...
      return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-4">
           {type !== 'Customer' && (<div className="bg-slate-50 border-b border-slate-200 p-2 flex justify-end"><select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="bg-white border border-slate-200 rounded px-2 py-1 text-xs font-bold text-slate-600 outline-none"><option value="profit">Sort: Highest Profit</option><option value="revenue">Sort: Highest Value</option><option value="qty">Sort: Highest Qty</option></select></div>)}
           <div className="overflow-x-auto">
             <table className="w-full text-left text-xs whitespace-nowrap">
                <thead className="bg-slate-50 text-slate-500 uppercase font-bold tracking-wider border-b border-slate-200"><tr><th className="p-4">{type}</th>{type !== 'Customer' && <th className="p-4 text-center">Qty</th>}<th className="p-4 text-right">Revenue</th><th className="p-4 text-right text-emerald-600">Gross</th></tr></thead>
                <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                  {arr.map((row, i) => (<tr key={i} className="hover:bg-slate-50"><td className="p-4">{row.key} {row.company ? <span className="text-[9px] text-slate-400 block mt-0.5 uppercase tracking-widest">{row.company}</span> : ''}</td>{type !== 'Customer' && <td className="p-4 text-center bg-slate-50/50">{row.qty}</td>}<td className="p-4 text-right text-slate-800 font-bold">Rs. {(row.revenue || row.productRevenue).toLocaleString()}</td><td className="p-4 text-right text-emerald-600 font-bold">Rs. {row.profit.toLocaleString()}</td></tr>))}
                </tbody>
             </table>
           </div>
        </div>
    ...
    return (
      <div className="h-full flex flex-col p-4">
        <div className="flex gap-2 mb-3 overflow-x-auto scrollbar-hide shrink-0">
           <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-lg border border-slate-200 shadow-sm shrink-0"><Calendar size={14} className="text-indigo-500" /><select value={dateFilter} onChange={e => setDateFilter(e.target.value)} className="bg-transparent font-bold text-[11px] text-slate-700 outline-none cursor-pointer"><option>Today</option><option>This Week</option><option>This Month</option><option>This Year</option><option>All Time</option></select></div>
           <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-lg border border-slate-200 shadow-sm shrink-0"><Filter size={14} className="text-slate-400" /><select value={filterCompany} onChange={e => setFilterCompany(e.target.value)} className="bg-transparent font-bold text-[11px] text-slate-700 outline-none cursor-pointer w-24"><option value="">All Brands</option>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
           <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-lg border border-slate-200 shadow-sm shrink-0"><Users size={14} className="text-slate-400" /><select value={filterCustomer} onChange={e => setFilterCustomer(e.target.value)} className="bg-transparent font-bold text-[11px] text-slate-700 outline-none cursor-pointer w-24"><option value="">All Clients</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-hide shrink-0">
           {['Overview', 'By Product', 'By Company', 'By Customer', 'Receivables'].map(v => (<button key={v} onClick={() => setView(v)} className={`px-4 py-2 rounded-xl font-bold text-xs whitespace-nowrap shadow-sm transition-colors ${view === v ? 'bg-slate-800 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}>{v}</button>))}
        </div>
        <div className="flex justify-between items-center bg-indigo-50/50 border border-indigo-100 p-2 rounded-xl mb-1 shrink-0">
            <span className="text-xs font-bold text-indigo-700 ml-2 uppercase tracking-widest">Share Report</span>
            <div className="flex gap-2">
               <button onClick={()=>handleExport('text')} title="Copy Text" className="p-2 bg-white text-slate-600 rounded-lg shadow-sm border border-slate-200"><Copy size={16}/></button>
               {view !== 'Overview' && <button onClick={()=>handleExport('csv')} title="Export CSV" className="p-2 bg-white text-slate-600 rounded-lg shadow-sm border border-slate-200"><Download size={16}/></button>}
               <button onClick={()=>handleExport('pdf')} title="Print / PDF" className="p-2 bg-indigo-600 text-white rounded-lg shadow-sm"><Printer size={16}/></button>
            </div>
        </div>
        <div className="flex-1 overflow-y-auto pr-1 pb-24 mt-2">
          {view === 'Overview' && (
            <div className="space-y-4">
               <div className="grid grid-cols-2 gap-3">
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm"><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Sales Trend</p><div className="flex items-center gap-2 mt-1"><span className={`text-lg font-black ${Number(reportEngine.trends.revenue) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{Number(reportEngine.trends.revenue) >= 0 ? '+' : ''}{reportEngine.trends.revenue}%</span><span className="text-xs text-slate-400">vs last period</span></div></div>
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm"><p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Profit Trend</p><div className="flex items-center gap-2 mt-1"><span className={`text-lg font-black ${Number(reportEngine.trends.profit) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{Number(reportEngine.trends.profit) >= 0 ? '+' : ''}{reportEngine.trends.profit}%</span><span className="text-xs text-slate-400">vs last period</span></div></div>
               </div>
               {Object.keys(reportEngine.dailyBreakdown).length > 0 && (
                 <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                   <p className="text-[10px] font-bold uppercase text-slate-500 tracking-wider mb-3">Daily Sales Trend</p>
                   <div className="flex items-end gap-1 h-24 overflow-x-auto">
                     {Object.entries(reportEngine.dailyBreakdown).sort((a,b) => a[0].localeCompare(b[0])).slice(-14).map(([date, data]) => {
                       const maxRevenue = Math.max(...Object.values(reportEngine.dailyBreakdown).map(d => d.revenue));
                       const height = maxRevenue > 0 ? (data.revenue / maxRevenue * 100) : 0;
                       return (<div key={date} className="flex flex-col items-center min-w-[28px]"><div className="relative w-full flex justify-center"><div className="w-5 bg-indigo-500 rounded-t" style={{height: `${Math.max(height, 4)}%`}} title={`Rs. ${data.revenue.toLocaleString()}`}></div></div><span className="text-[8px] text-slate-400 mt-1">{date.slice(-2)}</span></div>);
                     })}
                   </div>
                 </div>
               )}
               <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                 <p className="text-[10px] font-bold uppercase text-slate-500 tracking-wider mb-3">Top 5 Products by Profit</p>
                   ...
                   {Object.entries(reportEngine.byProduct).sort((a,b) => b[1].profit - a[1].profit).slice(0, 5).map(([name, data], i) => (
                       <div key={name} className="flex items-center gap-3">
                         ...
                         <div className="flex-1">
                           <div className="flex justify-between text-xs"><span className="font-semibold text-slate-700">{name}</span><span className="font-bold text-emerald-600">Rs. {data.profit.toLocaleString()}</span></div>
                           <div className="w-full bg-slate-100 rounded-full h-1.5 mt-1"><div className="bg-emerald-500 h-1.5 rounded-full" style={{width: `${data.profit > 0 ? Math.min((data.profit / (Object.entries(reportEngine.byProduct).sort((a,b) => b[1].profit - a[1].profit)[0]?.[1].profit || 1)) * 100, 100) : 0}%`}}></div></div>
                         </div>
                       </div>
                     ))}
                 </div>
               ...
               <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6 rounded-3xl shadow-xl border border-slate-800">
                  <p className="text-[10px] uppercase font-bold text-slate-400 mb-5 tracking-widest flex justify-between"><span>P&L Dashboard</span><span className="text-indigo-300">{dateFilter}</span></p>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center text-sm font-medium"><span className="text-slate-300">Gross Product Sales</span> <span className="font-bold text-white text-base">Rs. {reportEngine.kpis.productRevenue.toLocaleString()}</span></div>
                    <div className="flex justify-between items-center text-sm font-medium"><span className="text-rose-300">Total COGS (Cost)</span> <span className="font-bold text-rose-300">- Rs. {reportEngine.kpis.totalCOGS.toLocaleString()}</span></div>
                    ...
                    <div className="flex justify-between items-center text-xs font-medium"><span className="text-slate-400">Delivery Billed to Customers</span> <span className="font-bold text-slate-300">+ Rs. {reportEngine.kpis.deliveryBilled.toLocaleString()}</span></div>
                    <div className="flex justify-between items-center text-xs font-medium"><span className="text-rose-400">Driver Transport Expenses</span> <span className="font-bold text-rose-400">- Rs. {reportEngine.kpis.transportExpense.toLocaleString()}</span></div>
                    <div className="flex justify-between items-center text-xs font-medium"><span className="text-rose-400">General Operational Expenses</span> <span className="font-bold text-rose-400">- Rs. {reportEngine.kpis.totalExpenses.toLocaleString()}</span></div>
                    <div className="h-px bg-slate-700 my-2"></div>
                    <div className="flex justify-between items-center"><span className="font-bold uppercase tracking-widest text-emerald-400 text-xs">Total Net Profit</span><span className="font-black text-3xl text-emerald-400 tracking-tight">Rs. {reportEngine.kpis.netProfit.toLocaleString()}</span></div>
                  </div>
               </div>
               {view === 'Overview' && (
          ...
          {view === 'Receivables' && (
             <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mt-4">
                <h3 className="font-bold text-slate-800 mb-4 border-b border-slate-100 pb-3 text-sm tracking-wide">Aging Receivables Report</h3>
                <div className="space-y-3.5">
                  {reportEngine.receivablesList.map((r, i) => (<div key={i} className="flex justify-between items-center"><span className="font-semibold text-sm text-slate-700">{r.name}</span><span className="font-extrabold text-rose-600 text-sm">Rs. {r.amount.toLocaleString()}</span></div>))}
                </div>
             </div>
          )}
        </div>
    ...
    return (
      <div className="h-full flex flex-col p-4 pb-24">
         <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-6">
            <div className="flex justify-between items-center mb-3"><h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Record New Expense</h3><button onClick={() => setShowExpenseCatModal(true)} className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md uppercase tracking-wider">Manage Labels</button></div>
            ...
            <button onClick={saveExpense} className="w-full bg-rose-500 hover:bg-rose-600 text-white font-bold py-3.5 rounded-xl shadow-md">Record Expense</button>
         </div>
         <div className="flex-1 overflow-y-auto pr-1">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest mb-3 border-b border-slate-200 pb-2">Expense History</h3>
            <div className="space-y-3">
               {expenses.slice().reverse().map(exp => (
                 <div key={exp.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
                    <div><p className="font-bold text-slate-800 text-sm flex items-center gap-1.5"><Tag size={12} className="text-slate-400"/> {exp.category}</p><p className="text-[11px] text-slate-500 font-medium mt-0.5">{formatDateDisp(exp.date)} \u2022 {exp.note}</p></div>
                    <div className="text-right"><p className="font-extrabold text-rose-600 text-base">Rs. {exp.amount.toLocaleString()}</p><button onClick={async ()=>{ if(window.confirm("Delete expense?")) await deleteFromFirebase('expenses', exp.id) }} className="text-[10px] text-slate-400 hover:text-rose-500 font-bold uppercase mt-1">Delete</button></div>
                 </div>
               ))}
            </div>
         </div>
  ...
  const BulkOpsView = () => {
    ...
    const handleBulkSave = async () => {
       ...
       showToast(`Updated ${updatedCount} items prospectively!`);
    };
    ...
    const handleImportCSV = (e) => {
       ...
       reader.onload = async (event) => {
          ...
          showToast(`Success! ${addedCount} added, ${updatedCount} updated.`);
       };
    ...
    return (
      <div className="h-full flex flex-col p-4 pb-24">
         <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-4">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest mb-3 flex items-center gap-1.5"><Download size={14} className="text-indigo-600"/> Export Data</h3>
            <div className="flex gap-3">
               <button onClick={() => { const data = products.map(p => ({ ID: p.id, Name: p.name, Company: getCompanyName(p.companyId), Unit: p.unit, BoxQty: p.unitsInBox, Cost: p.costPrice, Selling: p.sellingPrice, Status: p.available ? 'In Stock' : 'Out' })); exportToCSV(data, 'Products_Master.csv'); }} className="flex-1 bg-indigo-50 border border-indigo-100 text-indigo-700 py-2.5 rounded-xl font-bold text-xs shadow-sm">Export Items</button>
               <button onClick={() => { const data = customers.map(c => ({ ID: c.id, Name: c.name, Contact: c.contactPerson, Phone: c.phone, Address1: c.address1, Map1: c.map1 })); exportToCSV(data, 'Customers_Master.csv'); }} className="flex-1 bg-indigo-50 border border-indigo-100 text-indigo-700 py-2.5 rounded-xl font-bold text-xs shadow-sm">Export Clients</button>
            </div>
         </div>
         <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-4">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest mb-2 flex items-center gap-1.5"><Upload size={14} className="text-emerald-600"/> Import Inventory</h3>
            <p className="text-[10px] text-slate-500 mb-3 leading-tight">Upload a CSV file to add new items or update prices. Existing items with the exact same name will be updated.</p>
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 mb-3">
               <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Required CSV Columns:</span>
               <code className="text-[10px] font-mono text-indigo-600 font-bold tracking-tight">Name, Company, Unit, BoxQty, Cost, Selling</code>
            </div>
            <div className="flex gap-3">
               <button onClick={downloadImportTemplate} className="flex-1 bg-white border border-slate-300 text-slate-700 py-2.5 rounded-xl font-bold text-xs shadow-sm">Get Template</button>
               <label className="flex-[1.5] bg-emerald-500 text-white py-2.5 rounded-xl font-bold text-xs shadow-sm cursor-pointer text-center flex items-center justify-center"><input type="file" accept=".csv" className="hidden" onChange={handleImportCSV} />Select & Import CSV</label>
            </div>
         </div>
         <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
               <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2"><ArrowUpDown size={16}/> Quick Price Edit</h3>
               <button onClick={handleBulkSave} className="bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold text-xs shadow-sm">Save All</button>
            </div>
            <div className="flex-1 overflow-auto p-2">
               <table className="w-full text-left text-xs whitespace-nowrap min-w-[800px]">
                  <thead className="text-slate-500 uppercase font-bold tracking-wider"><tr><th className="p-2 pb-3">Product Name</th><th className="p-2 pb-3 w-20">Unit</th><th className="p-2 pb-3 w-20">Box Qty</th><th className="p-2 pb-3 w-24">Cost</th><th className="p-2 pb-3 w-24">Selling</th><th className="p-2 pb-3 w-20 text-center">Active</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                     {bulkProducts.map(p => (
                        <tr key={p.id} className="hover:bg-slate-50">
                           <td className="p-2"><input type="text" className="w-full p-1.5 bg-slate-50 border border-slate-200 rounded font-semibold text-slate-800 outline-none focus:border-indigo-400" value={p.name} onChange={e => setBulkProducts(bulkProducts.map(bp=>bp.id===p.id?{...bp, name: e.target.value}:bp))} /></td>
  ...
  // --- Main Render ---
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-[Inter,system-ui,sans-serif] flex flex-col max-w-md mx-auto relative shadow-2xl overflow-x-hidden print:hidden">
