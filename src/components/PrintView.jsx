...
`;
      ...
      if (data.items && Array.isArray(data.items)) {
        data.items.forEach(i => {
           text += `- ${i.name || 'Unknown'} ${i.isBonus ? '(BONUS) ' : ''}(x${i.quantity || 0}): ${i.isBonus ? 'FREE' : 'Rs.' + ((i.price || 0) * (i.quantity || 0))}\
`;
           if (i.isBonus) savings += ((i.originalPrice || 0) * (i.quantity || 0));
        });
      }
      if (data.deliveryBilled > 0) text += `Delivery: Rs.${data.deliveryBilled}\
`;
      text += `\
*Current Bill: Rs.${data.total || 0}*\
`;
      if (savings > 0) text += `*Total Savings: Rs.${savings}*\
`;
...
`;
      text += `\
*Vehicle/Transport:* ${data.vehicle || 'N/A'}\
`;
      if(data.vehicle === 'Intercity Transport') {
         text += `Transport Co: ${data.transportCompany || 'N/A'}\
Bilty No: ${data.biltyNumber || 'N/A'}\
`;
...
`;
      text += `*Remaining Balance: Rs.${data.newBalance || 0}*\
\
Thank you for your business!`;
    } else if (docType === 'ledger') {
       text += `*ACCOUNT STATEMENT*\
Customer: ${data.customerName}\
Statement Period: ${formatDateDisp(data.dateRange?.start)} to ${formatDateDisp(data.dateRange?.end)}\
\
`;
       text += `Total Debits: Rs.${data.totalDebit || 0}\
Total Credits: Rs.${data.totalCredit || 0}\
*Current Closing Balance: Rs.${data.closingBal || 0}*\
\
Please arrange payment at your earliest convenience.`;
    } else if (docType === 'report') {
       text += `*${data.title || 'Report'}*\
Period: ${data.dateFilter || 'All Time'}\
\
`;
...
`;
  ...
  const handlePDF = () => {
    ...
    if (isThermal) {
      ...
      const opt = {
        ...
        jsPDF: { 
          unit: 'mm', 
          format: [80, 400], // Wide enough height to avoid cutting - will trim
  ...
  return (
    <div className="fixed inset-0 bg-white z-[100] overflow-auto" id="print-root">
